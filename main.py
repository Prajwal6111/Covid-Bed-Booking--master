...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME=vishal['gmail-user'],
    MAIL_PASSWORD=vishal['gmail-password']
)
mail = Mail(app)
...
def admin():
    ...
    if request.method=="POST":
        ...
        password=request.form.get('password')
        if(username==vishal['user'] and password==vishal['password']):
...
def hospitalUser():
    ...
    if('user' in session and session['user']==vishal['user']):
        ...
        if request.method=="POST":
            ...
            mail.send_message('COVID CARE CENTER',sender=vishal['gmail-user'],recipients=[email],body=f"Welcome thanks for choosing us\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You..." )
...
def slotbooking():
    ...
    if request.method=="POST":
        ...
        if not check2:
            flash("Hospital Code Not Exists","warning")
        ...
        if(seat>0 and check):
            ...
            db.session.add(res)
            db.session.commit()
            flash("Slot is Booked Kindly Visit for further Procedure","success")
        else:
